...
docker run -d \
  ...
  ghcr.io/0xdc05f/livescreenlog:0.3.1
```

이미지:
- GHCR 패키지에 Actions Write가 연결된 뒤: `ghcr.io/0xdc05f/livescreenlog:0.3.1`
- 그 전: `./gradlew bootJar` 후 `deploy/docker-compose.yml` 로컬 빌드
