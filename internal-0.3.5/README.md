# LiveScreenLog 0.3.5 개발계

공유 compose의 postgres / redis 를 사용한다. 이 폴더의 `docker-compose.yml` 은 단독 스택이 아니라 `/app/docker-compose.yml` 에 붙일 서비스 샘플이다.

이번 업데이트는 **서버(Docker 이미지)만**. Vue SDK API는 0.3.4와 동일하다. 클라이언트 `livescreenlog@0.3.4` 를 그대로 써도 되고, **필수 교체는 docker 이미지**다.
...
| 파일 | 종류 | docker load |
|------|------|-------------|
| `docker-images/livescreenlog-0.3.5-image.tar.gz` | Docker 이미지 | **이 파일만** |
| `images/livescreenlog-0.3.5.tar.gz` | 동일 이미지 | 가능 |

npm 트리는 생략 (Vue SDK는 0.3.4와 동일). `docker load` 는 `*-image.tar.gz` 만.
...
```bash
docker run --rm --entrypoint id livescreenlog:0.3.5
...
```bash
docker load -i docker-images/livescreenlog-0.3.5-image.tar.gz
docker tag ghcr.io/0xdc05f/livescreenlog:0.3.5 livescreenlog:0.3.5
```
...
Vue SDK는 0.3.4와 같다. npm 재설치는 필수가 아니다. 버전만 맞추려면:

```bash
npm i livescreenlog@0.3.5 --registry https://nexus.example/repository/npm-hosted/
```
