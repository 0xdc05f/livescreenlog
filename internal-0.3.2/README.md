...
| 파일 | 종류 | docker load |
|------|------|-------------|
| `docker-images/livescreenlog-0.3.2-image.tar.gz` | Docker 이미지 | **이 파일만** |
| `images/livescreenlog-0.3.2.tar.gz` | 동일 이미지 | 가능 |
| `files/livescreenlog-0.3.2.tgz` | npm 패키지 | **하면 안 됨** |
...
```bash
docker run --rm --entrypoint id livescreenlog:0.3.2
...
```bash
docker load -i docker-images/livescreenlog-0.3.2-image.tar.gz
