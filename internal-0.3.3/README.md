...
| 파일 | 종류 | docker load |
|------|------|-------------|
| `docker-images/livescreenlog-0.3.3-image.tar.gz` | Docker 이미지 | **이 파일만** |
| `images/livescreenlog-0.3.3.tar.gz` | 동일 이미지 | 가능 |
| `files/livescreenlog-0.3.3.tgz` | npm 패키지 | **하면 안 됨** |
...
```bash
docker run --rm --entrypoint id livescreenlog:0.3.3
...
```bash
docker load -i docker-images/livescreenlog-0.3.3-image.tar.gz
docker tag ghcr.io/0xdc05f/livescreenlog:0.3.3 livescreenlog:0.3.3
```
...
- 컨테이너 간 DB 포트는 **5432**. 호스트 매핑 15432는 쓰지 않음. 앱 포트는 `8090:8080` (톰캣 8080).
...
```bash
npm i livescreenlog@0.3.3 --registry https://nexus.example/repository/npm-hosted/
```
